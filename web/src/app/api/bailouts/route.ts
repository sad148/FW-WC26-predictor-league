...
/** POST /api/bailouts — trade 10 trivia/bracket pts for 100 coins, scoped to active league. */
export async function POST() {
  try {
      ...
      SELECT
        (
          ...
          + COALESCE((
              ...
              FROM bets WHERE user_id = ${session.userId} AND league_id = ${leagueId}
            ), 0)
          + COALESCE((SELECT SUM(coins_awarded) FROM bailouts WHERE user_id = ${session.userId} AND league_id = ${leagueId}), 0)
        )::int AS coins,
        (
          COALESCE((SELECT SUM(points_awarded) FROM question_answers WHERE user_id = ${session.userId} AND league_id = ${leagueId} AND outcome = 'win'), 0)
          + COALESCE((SELECT SUM(points_awarded) FROM bracket_picks   WHERE user_id = ${session.userId} AND league_id = ${leagueId} AND outcome = 'win'), 0)
          + COALESCE((SELECT SUM(points_awarded) FROM group_picks     WHERE user_id = ${session.userId} AND league_id = ${leagueId}), 0)
          - COALESCE((SELECT SUM(points_deducted) FROM bailouts       WHERE user_id = ${session.userId} AND league_id = ${leagueId}), 0)
        )::int AS "availablePts"
    `);
    ...
    const [row] = await db.insert(bailouts).values({ userId: session.userId!, leagueId }).returning();
