...
/** GET /api/answers — list current user's trivia answers for the active league. */
export async function GET() {
  try {
    const session = await requireActiveLeague();
    const rows = await db.select().from(questionAnswers)
      .where(and(
        eq(questionAnswers.userId, session.userId!),
        eq(questionAnswers.leagueId, session.activeLeagueId),
      ))
...
/** POST /api/answers — submit or replace an answer for the active league. */
export async function POST(req: NextRequest) {
  try {
    ...
    const [row] = await db.insert(questionAnswers).values({
      ...
      target: [questionAnswers.userId, questionAnswers.questionId, questionAnswers.leagueId],
