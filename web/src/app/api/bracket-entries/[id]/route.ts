...
export async function PATCH(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (has('correctPick')) updates.correctPick = body.correctPick == null || body.correctPick === ''
                                                    ? null
                                                    : String(body.correctPick);
    if (has('status'))      updates.status      = String(body.status);

    const [row] = await db.update(bracketEntries).set(updates).where(eq(bracketEntries.id, eid)).returning();
    ...
    if (row.status === 'settled' && row.correctPick) {
      ...
      const picks = await db.select().from(bracketPicks).where(eq(bracketPicks.entryId, eid));
      for (const p of picks) {
        if (p.outcome !== 'pending') continue;
        const correct = p.pick.trim().toLowerCase() === expected;
        await db.update(bracketPicks).set({
