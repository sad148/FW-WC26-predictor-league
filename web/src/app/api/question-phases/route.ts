...
import { questionPhases, audit } from '@/db/schema';
...
/**
 * GET /api/question-phases — the answer window for each trivia phase. Open to anyone.
...
/**
 * PUT /api/question-phases — admin sets one phase's window. Upsert keyed on phase.
 * Empty/missing times clear the window (phase reverts to "not set" → no answers accepted).
 */
export async function PUT(req: NextRequest) {
  try {
    ...
    const [row] = await db
      ...
      .onConflictDoUpdate({ target: questionPhases.phase, set: { startTime, endTime } })
      .returning();

    await db.insert(audit).values({
      action: 'setQuestionPhase',
